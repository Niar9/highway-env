...
class HighwayEnv(AbstractEnv):
    ...
    def _reward(self, action: Action) -> float:
        ...
        reward = \
            + self.config["collision_reward"] * self.vehicle.crashed -1 \
            + self.config["lane_change_reward"] * lane_change 0.4 \
            + self.config["right_lane_reward"] * lane / max(len(neighbours) 0.8) \
